...
func TestPipeFormatUpdateNeededFields(t *testing.T) {
	...
	f(`format if (f4:z) "<f3>foo" as x`, "*", "f1,f2", "*", "f1,f2")
	f(`format if (f1:z) "<f3>foo" as x`, "*", "f1,f2", "*", "f2")
	...
	f(`format if (f3:z) "<f1>foo" as f2`, "f1,f2,y", "", "f1,f2,f3,y", "")
